...
export const TextInput = ({ label, status = 'default' }) => (
...
export const Calendar = () => {
  ...
  return <div className={styles.calendar}>{days.map((d) => <button key={d} type="button" className={active === d ? styles.active : ''} onClick={() => setActive(d)}>{d}</button>)}</div>;
};
